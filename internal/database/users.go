...
func (s *Store) ListUsers(ctx context.Context) ([]domain.User, error) {
	rows, err := s.pool.Query(ctx, `SELECT u.id,u.email,u.display_name,u.password_hash,u.role,u.enabled,u.created_at,u.updated_at,u.last_login_at,EXISTS(SELECT 1 FROM user_mfa m WHERE m.user_id=u.id AND m.enabled_at IS NOT NULL) FROM users u ORDER BY lower(u.email),u.id`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.ID, &user.Email, &user.DisplayName, &user.PasswordHash, &user.Role, &user.Enabled, &user.CreatedAt, &user.UpdatedAt, &user.LastLoginAt, &user.MFAEnabled); err != nil {
...
func (s *Store) UpdateUser(ctx context.Context, id, email, displayName string, enabled bool, now time.Time, event domain.AuditEvent) (domain.User, error) {
	...
	err = tx.QueryRow(ctx, `SELECT u.id,u.email,u.display_name,u.password_hash,u.role,u.enabled,u.created_at,u.updated_at,u.last_login_at,EXISTS(SELECT 1 FROM user_mfa m WHERE m.user_id=u.id AND m.enabled_at IS NOT NULL) FROM users u WHERE u.id=$1 FOR UPDATE`, id).Scan(&current.ID, &current.Email, &current.DisplayName, &current.PasswordHash, &current.Role, &current.Enabled, &current.CreatedAt, &current.UpdatedAt, &current.LastLoginAt, &current.MFAEnabled)
	if err != nil {
